...
public class WXViewUrlInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (weixinfake){
            WXConfigUtil.checkWXMenuCallBackParse(request,true);
            return true;
        }
        Map<String, String[]> parameterMap = request.getParameterMap();

        Boolean checkresult = WXConfigUtil.checkWXMenuCallBackParse(request,false);

        if (checkresult){
            WXBindInfo wxBindInfo = wxBindService.getUserByWXOpenID((String) request.getSession().getAttribute("wx_open_id"));
            if(wxBindInfo == null){
